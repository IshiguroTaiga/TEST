...
const TOURISM_SPOTS: TourismSpot[] = [
  ...
  {name: 'Cape Bojeador Lighthouse', category: 'Historical', description: 'Spanish-era lighthouse overlooking Burgos, offering sea views.', map_link: 'https://maps.google.com/?q=Cape+Bojeador+Lighthouse'},
  {name: 'Kapurpurawan Rock Formation', category: 'Natural Wonder', description: 'White limestone formations sculpted by nature.', map_link: 'https://maps.google.com/?q=Kapurpurawan+Rock+Formation'},
  {name: 'Pagudpud Beach', category: 'Beach', description: 'White sand and blue waters – a must-visit for relaxation.', map_link: 'https://maps.google.com/?q=Pagudpud+Beach'},
  {name: 'Bangui Windmills', category: 'Landmark', description: 'Iconic wind turbines along Bangui Bay.', map_link: 'https://maps.google.com/?q=Bangui+Windmills'},
  {name: 'Batac Riverside Empanadaan', category: 'Food Spot', description: 'Stalls serving authentic Ilocano empanada.', map_link: 'https://maps.google.com/?q=Batac+Riverside+Empanadaan'},
  {name: 'Paoay Sand Dunes', category: 'Adventure', description: 'Try 4x4 rides and sandboarding on the iconic dunes.', map_link: 'https://maps.google.com/?q=Paoay+Sand+Dunes'},
  {name: 'Malacanang of the North', category: 'Historical', description: 'Former presidential rest house turned museum.', map_link: 'https://maps.google.com/?q=Malacanang+of+the+North'},
  {name: 'Sinking Bell Tower', category: 'Historical', description: 'Centuries-old bell tower slowly sinking into the ground.', map_link: 'https://maps.google.com/?q=Sinking+Bell+Tower'},
  {name: 'La Paz Sand Dunes', category: 'Adventure', description: 'Site for off-road and ATV adventures near Laoag.', map_link: 'https://maps.google.com/?q=La+Paz+Sand+Dunes'},
  {name: 'Adams Rainforest', category: 'Nature', description: 'Tranquil mountain village with lush forests.', map_link: 'https://maps.google.com/?q=Adams+Ilocos+Norte'},
  {name: 'Patapat Viaduct', category: 'Landmark', description: 'Scenic coastal bridge connecting Ilocos and Cagayan.', map_link: 'https://maps.google.com/?q=Patapat+Viaduct'},
  {name: 'Blue Lagoon', category: 'Beach', description: 'Also known as Maira-ira Beach, famous for turquoise waters.', map_link: 'https://maps.google.com/?q=Blue+Lagoon'},
  {name: 'Madongan Dam', category: 'Nature', description: 'Scenic river dam in Dingras, popular for swimming.', map_link: 'https://maps.google.com/?q=Madongan+Dam'},
  {name: 'San Nicolas Pottery Village', category: 'Cultural', description: 'Home of traditional Ilocano pottery.', map_link: 'https://maps.google.com/?q=San+Nicolas+Pottery'},
  {name: 'Juan Luna Shrine', category: 'Historical', description: 'Ancestral house of the famous Filipino painter.', map_link: 'https://maps.google.com/?q=Juan+Luna+Shrine'},
  {name: 'Dingras Church Ruins', category: 'Historical', description: 'Historic 18th-century church ruins.', map_link: 'https://maps.google.com/?q=Dingras+Church+Ruins'},
  {name: 'Sarrat Church', category: 'Historical', description: 'Baroque church, site of the Marcos-Romualdez wedding.', map_link: 'https://maps.google.com/?q=Sarrat+Church'},
  {name: 'Paoay Lake', category: 'Nature', description: 'Protected national lake near Malacanang of the North.', map_link: 'https://maps.google.com/?q=Paoay+Lake'},
  {name: 'Fort Ilocandia', category: 'Accommodation', description: 'Premier beachfront resort with casino and spa.', map_link: 'https://maps.google.com/?q=Fort+Ilocandia'},
  // ... including internal AI awareness for all 85 legacy spots
];
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div onClick={() => setActiveTab('settings')} className="flex items-center space-x-2 bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-full border border-white/10 cursor-pointer transition-all">
              <div className="w-6 h-6 bg-mmsu-gold rounded-full flex items-center justify-center text-mmsu-green font-black text-[10px] uppercase">{user.name.charAt(0)}</div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-t border-gray-200 dark:border-gray-800 z-40 md:relative md:bg-transparent md:border-none">
        <div className="max-w-xl mx-auto flex justify-around py-3">
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id as Tab)} className={`flex flex-col items-center space-y-1 transition-all ${activeTab === tab.id ? 'text-mmsu-green dark:text-mmsu-gold' : 'text-gray-400'}`}>
  ...
  <div className="space-y-10">
    <section className="mmsu-gradient text-white p-10 md:p-16 rounded-[3rem] shadow-2xl relative overflow-hidden">
      <div className="relative z-10 space-y-6">
        ...
        <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-none">Rise Higher, <br/><span className="text-mmsu-gold italic">Stallion {user.name.split(' ')[0]}!</span></h2>
        <p className="opacity-80 text-sm md:text-lg font-medium max-w-lg">Centralized hub for {user.college} students. Grounded in university data and Ilocano heritage.</p>
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          Knowledge: You have deep info on MMSU academics AND the 85 Ilocos Norte tourism spots (Paoay, Pagudpud, Bangui, etc.).
          Rules: Ground your responses in reality. Provide links to Google Maps for locations. Use legacy "UmaAI" personality when asked about tourism.`,
          tools: [{ googleSearch: {} }]
        }
      });
    ...
    <div className="flex flex-col h-[75vh] bg-white dark:bg-gray-900 rounded-[3rem] shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
      <div className={`p-6 flex items-center justify-between transition-colors ${mode === 'TUTORING' ? 'bg-mmsu-gold text-mmsu-green' : 'bg-mmsu-green text-white'}`}>
        <div className="flex items-center space-x-4">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transform -rotate-3 ${mode === 'TUTORING' ? 'bg-mmsu-green text-white' : 'bg-mmsu-gold text-mmsu-green'}`}><i className={`fas ${mode === 'TUTORING' ? 'fa-user-graduate' : 'fa-robot'} text-xl`}></i></div>
          <div>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-gray-50/50 dark:bg-gray-950/20 no-scrollbar">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] px-6 py-4 rounded-[2rem] text-sm shadow-sm ${m.role === 'user' ? 'bg-mmsu-green text-white rounded-tr-none' : 'bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-tl-none'}`}><p className="whitespace-pre-wrap leading-relaxed">{m.text}</p></div>
          </div>
        ))}
      ...
      <div className="p-6 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
        <div className="flex gap-3">
          <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="Inquire about academics, policies, or Ilocos..." className="flex-1 bg-gray-100 dark:bg-gray-950 border-none rounded-2xl px-6 py-4 text-sm focus:ring-2 focus:ring-mmsu-green outline-none dark:text-white" />
    ...
    <div className="space-y-10 animate-fadeIn">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input type="text" placeholder="Search spots..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-14 pr-6 py-4 rounded-[1.5rem] bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm outline-none focus:ring-2 focus:ring-mmsu-green dark:text-white transition-all" />
        </div>
      </div>
      <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
        {categories.map(c => (
          <button key={c} onClick={() => setFilter(c)} className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${filter === c ? 'bg-mmsu-green text-white shadow-lg scale-105' : 'bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 hover:border-mmsu-green'}`}>{c}</button>
        ))}
      </div>
...
const CourseExplorer = ({ user }: any) => {
  ...
  return (
    <div className="space-y-10 animate-fadeIn">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div><h2 className="text-3xl font-black uppercase tracking-tight">Academic Prospectus</h2><p className="text-xs text-mmsu-green dark:text-mmsu-gold font-bold uppercase tracking-widest mt-2">Department: {user.college}</p></div>
        <div className="relative w-full md:w-80"><i className="fas fa-search absolute left-5 top-1/2 -translate-y-1/2 text-gray-400"></i><input type="text" placeholder="Search catalog..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-14 pr-6 py-4 rounded-[1.5rem] bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm outline-none focus:ring-2 focus:ring-mmsu-green dark:text-white transition-all" /></div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filtered.map((c, i) => (
          <div key={i} className="bg-white dark:bg-gray-900 p-10 rounded-[3rem] border border-gray-100 dark:border-gray-800 shadow-sm hover:border-mmsu-gold transition-all"><span className="text-[10px] font-black bg-mmsu-gold/10 text-mmsu-green dark:text-mmsu-gold px-4 py-2 rounded-xl border border-mmsu-gold/20 uppercase tracking-widest mb-6 inline-block">{c.code}</span><h4 className="font-black text-xl mb-4">{c.title}</h4><div className="flex justify-between items-center pt-8 border-t border-gray-50 dark:border-gray-800"><span className="text-[10px] text-gray-400 font-black uppercase tracking-widest">{c.credits} Units</span><button className="text-[10px] font-black uppercase text-mmsu-green dark:text-mmsu-gold hover:underline">Syllabus Details</button></div></div>
        ))}
      </div>
    </div>
  );
...
const SettingsView = ({ user, setUser }: any) => {
  ...
  return (
    <div className="max-w-md mx-auto space-y-10 animate-fadeIn pt-10">
      <div className="text-center space-y-4"><div className="w-24 h-24 bg-mmsu-green rounded-[2.5rem] flex items-center justify-center text-white text-4xl mx-auto shadow-2xl"><i className="fas fa-id-card"></i></div><h2 className="text-3xl font-black tracking-tight uppercase">Stallion Profile</h2><p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.3em]">MMSU Student Digital ID</p></div>
      <div className="bg-white dark:bg-gray-900 p-10 rounded-[3.5rem] border border-gray-100 dark:border-gray-800 shadow-2xl space-y-8"><div className="space-y-2"><label className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-2">Display Name</label><input type="text" value={user.name} onChange={e => setUser({...user, name: e.target.value})} className="w-full p-5 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 font-bold focus:ring-2 focus:ring-mmsu-green outline-none dark:text-white transition-all" /></div><div className="space-y-2"><label className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-2">Assigned College</label><select value={user.college} onChange={e => setUser({...user, college: e.target.value})} className="w-full p-5 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 font-bold focus:ring-2 focus:ring-mmsu-green outline-none dark:text-white transition-all">{colleges.map(c => <option key={c} value={c}>{c}</option>)}</select></div><button className="w-full bg-mmsu-green text-white py-5 rounded-[1.5rem] font-black uppercase tracking-widest text-xs shadow-2xl shadow-mmsu-green/20 hover:scale-[1.02] transition-all active:scale-95">Update Profile</button></div>
    </div>
  );
};
