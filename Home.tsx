  ...
  return (
    <div className={`space-y-8 animate-fadeIn ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
      ...
      <section className="bg-gradient-to-br from-mmsu-green via-mmsu-darkGreen to-green-900 text-white p-8 md:p-12 rounded-[2rem] shadow-2xl relative overflow-hidden border border-white/10">
        ...
        <div className="absolute bottom-0 left-1/2 w-full h-1/2 bg-gradient-to-t from-black/20 to-transparent opacity-50"></div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <span className={`text-[10px] px-3 py-1 rounded-full font-black uppercase tracking-widest ${
                    ann.category === 'Scholarship' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' :
                    ann.category === 'Enrollment' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-end">
                  <span className="text-xs font-bold text-gray-400">Read Details <i className="fas fa-chevron-right ml-1"></i></span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className={`mt-8 p-6 rounded-3xl border-2 border-dashed flex flex-col items-center text-center space-y-4 ${
            isDark ? 'border-gray-700 bg-gray-900/20' : 'border-mmsu-gold/30 bg-mmsu-gold/5'
          }`}>
            <div className="w-12 h-12 bg-mmsu-gold rounded-full flex items-center justify-center text-mmsu-green">
              <i className="fas fa-question-circle text-2xl"></i>
            </div>
            <div>
              <h5 className={`font-bold text-sm ${isDark ? 'text-white' : 'text-gray-800'}`}>Need Help?</h5>
              <p className="text-[10px] text-gray-500 uppercase mt-1">Talk to our Stallion AI Assistant</p>
            </div>
            <button 
              onClick={onNavigateToChat}
              className="bg-mmsu-green text-white px-6 py-2 rounded-xl text-xs font-black hover:bg-mmsu-darkGreen transition-all w-full shadow-lg shadow-mmsu-green/20"
            >
              Start Chatting
            </button>
          </div>
